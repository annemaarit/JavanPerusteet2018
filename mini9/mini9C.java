...
Kuvaus:	Ohjelma pyytää ensin uusia asiakastietoja, uudet tiedot voi
		...
		tallentaa asiakkaan tiedot BINÄÄRItiedostoon. Ohjelma tulostaa 
		kaikkien asiakkaiden tiedot ja lopuksi kysyy etsittävän asiakkaan
		ID -tunnusta sekä tulostaa löydetyt tiedot.
		- jos binääritiedosto on jo olemassa, ohjelma kirjoittaa yli 
		  sen tiedot
...
class Asiakastiedosto implements Serializable{
	...
	Asiakastiedosto(String tdNimi){
		...
		kpl=0;							//pyytää asiakkaiden määrää
	}
	...
	public void tallennaUusiAsiakas(Asiakas as, ObjectOutputStream kirjoittaja){
		try{
			kirjoittaja.writeObject(as);		//olion tiedostoon sarjoitus
			kpl++;
			//System.out.println("Uusi asiakas nro: "+kpl+" lisätty");
		}
		catch (IOException ex){
			System.out.print("Asiakkaan sarjallistamisessa tapahtui virhe. Lopetetaan.");
			System.exit(1);			
		}	
	}
...
public class mini9C {
	...
	public static final String ASIAKASTIEDOSTO="asiakastiedotC.dat";	//asiakastiedoston nimi ja sijainti
	...
	public static void main(String [ ] args) {	
		...
		try{
			ObjectOutputStream olionKirjoittaja=new ObjectOutputStream(new FileOutputStream(ASIAKASTIEDOSTO));
		  do{
			do{
			...
			if (vastaus!=1){									//käyttäjä syöttää itse tiedot
				...
				asiakkaat.tallennaUusiAsiakas(asiakas, olionKirjoittaja);//asiakastietojen tallennus binääritiedostoon
				asiakas=null;
			}
			else{												//käytetään testiaineistoa
				System.out.print("Kuinka monta testiasiakasta luon?:  ");
				vastaus=lukija.nextInt();
				luoTestiAsiakkaita(asiakkaat,olionKirjoittaja,vastaus);	 //pyydetään testiasiakkaiden luomista
			}
			do{												
		...
		catch (IOException ex){								   //tiedoston lukuvirhe
			System.out.print("Tiedoston lukijan luomisessa tapahtui virhe. Lopetetaan.");
			System.exit(1);			
		}
	...
	public static void luoTestiAsiakkaita(Asiakastiedosto td, ObjectOutputStream kirjoittaja, int kpl){
			...
			for (int i=0;i<kpl;i++){											//toistetaan halutun määrän verran 
				...
				td.tallennaUusiAsiakas(testiasiakas, kirjoittaja);				//asiakastietojen tallennus binääritiedostoon
			}
	}
